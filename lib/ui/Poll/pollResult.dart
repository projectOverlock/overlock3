...
class _pollResultState extends State<pollResult> {
  ...
  Widget _buildBody(BuildContext context)
  {
    return StreamBuilder<QuerySnapshot>( // 데이터 읽어드리기, firebase에 데이터들이 있는데 이게 변할때 정보를 계속 읽어주는 역할 수행.
        stream: FirebaseFirestore.instance.collection('투표').snapshots(), //snapshot에 데이터 저장.(name, votes)
  ...
  Widget _buildListItem(BuildContext context, DocumentSnapshot doc)
  {
    ...
    // Navigator.pop(context, MaterialPageRoute( builder: (context) => pollPages(),));
